...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 embed_dim=1024, depth=24, num_heads=16, mlp_dim=None,
        ...
        if mlp_dim: 
            enc_mlp_ratio = mlp_dim / embed_dim
        else:
            enc_mlp_ratio = mlp_ratio

        self.blocks = nn.ModuleList([
            Block(embed_dim, num_heads, enc_mlp_ratio, qkv_bias=True, norm_layer=norm_layer)
        ...
        self.decoder_pred = nn.Linear(decoder_embed_dim, predict_feature_dim, bias=True) # decoder to patch
        # --------------------------------------------------------------------------
    ...
    def forward(self, imgs, clip_teacher, mask_ratio=0.75):
        latent, mask, ids_restore = self.forward_encoder(imgs, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        loss = self.forward_loss(clip_teacher, pred, mask)
        return loss
...
def mae_vit_large_patch16_dec512d4b(**kwargs):
    model = MaskedAutoencoderViT(
